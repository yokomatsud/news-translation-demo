...
The last step is to execute the command when the user clicks the command. We need to add an event handler with jQuery (jQuery Terminal dependency) or we can use the native browser `[addEventListener][41]`. Here we use jQuery:
...
If you would like to make command arguments in different colors, you'll need a function, where you will to use [String::replace][42].
...
You can read more about typing animation on a wiki article: [Typing Animation][44][.][45]
...
This is an example of another way to print something on the terminal, if you return something from a function it will be printed. You can also return a [Promise][46], so you can send an [AJAX][47] request to the server and print the results.
...
Here is a fully working Demo of our [Interactive Terminal Portfolio Website][48].
...
-   CodePen Collection with [jQuery Terminal Demos][49].
-   [Retro (Vintage) Terminal CodePen Demo][50].
-   [jQuery Terminal Examples Page][51].
-   [Terminal 404 Error Page][52].
-   [Fake GNU/Linux Terminal][53].

If you have some idea that is not listed here you can ask on [StackOverflow with jquery-terminal tag][54]. If you have something more time consuming, you can also ask for [paid support][55].

If you like this post, you can [check my website][56], (that has a terminal portfolio similar to this one, and also a chat), and [follow me on Twitter/X][57] and [LinkedIn][58].
